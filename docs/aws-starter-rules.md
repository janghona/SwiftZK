<!--
Fetched from the AWS Agent Toolkit setup (Step 7), "new AWS experience" ruleset:
  https://raw.githubusercontent.com/aws/agent-toolkit-for-aws/refs/heads/main/rules/aws-starter-rules.md

Stored here (not project-root CLAUDE.md) by user choice, so it does not override
the SwiftZK-Wallet paper instructions. To make an AI tool apply these rules
automatically, copy this content into that tool's rules file
(Claude Code: CLAUDE.md at the project root; Codex: AGENTS.md; etc.), or paste it
into a session when doing AWS work.
-->
